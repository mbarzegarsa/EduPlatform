class ProfessionalCertificateSystem {
        ...
        if (!this.validateCertificateData(data)) {
            ...
            requiredFields.forEach(field => {
                if (!data[field.id]) {
                    ...
                    fieldElement.className = 'missing-field';
                    fieldElement.innerHTML = `<i class="fas fa-exclamation-circle"></i> ${field.label}`;
                    missingFields.appendChild(fieldElement);
                }
            });
            return;
        }
    ...
    generateCertificateHTML(data, template = 'official') {
        if (this.currentDocumentType === 'letter') {
            ...
            return this.generateCertificateOriginalHTML(data, template);
        }
    }

    generateCertificateOriginalHTML(data, template = 'official') {
